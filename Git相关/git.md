...
- `rm -rf <dir_name>`  
删除非空目录  
-r参数:向下递归，不管有多少级目录，一并删除；  
-f参数：直接强行删除，不做任何提示；  
- `vim <file>`  
在编辑器中修改文件内容  
  1. __i__ 插入(进入编辑器编辑文件)  
  1. __Ese__ 退出编辑器  
  1. __:wq__ 保存并退出  
...
- `git commit --amend -m 'message'`  
  git文档翻译为：修改最后一次提交  
  在上一次提交的基础上提交(个人觉得也可以理解为合并上次的提交)，
  因为commit时带上`--amend`参数，在git log种只会看到一次commit操作的记录；
- `git reset HEAD <file>`  
  取消暂存的文件，及将add之后的文件返回到add之前的状态(工作区，Untracked files 未跟踪的文件)  
- `git checkout -- <file>`  
  撤销对文件的修改，注意：此命令只针对之前commit过的文件有效，即之前commit过，又修改后且还未add到缓存区，可以恢复到上次commit的状态；
...
- `git clone url`  
克隆远程仓库，__该命令不要求在`init`后的目录中使用，本地非git初始化目录也可使用，克隆完项目后会有一个默认名为origin的远程库__。  
- `git remote`  
列出每个远程库的简短名字。  
- `git remote -v`  
显示对应的克隆地址。  
...
把本地分支locan_branch_name推送到远程库，并在远程库中新建名为[remote_branch_name]的分支。  
...
不带任何参数的git push，默认只推送当前分支，这叫做 __simple方式__，  
还有一种 __matching方式__，会推送所有有对应的远程分支的本地分支，  
...
1. 按键盘字母 i 进入insert(输入)模式,编辑必要的注释 说明合并的原因,编辑的内在第一行显示,也可不修改  
...
利用ssh 及命令`git clone...`下载GitHub上的仓库报上述错误，  
且ssh公钥 已在GitHub保存，  
经 `ssh -T git@github.com`测试ssh是否配置成功，  
同样报`ssh: connect tohost github.com port 22:Connection timed out`错误，  
错误提示中有`22 端口`，暂未找到端口相关解决方式，  
可行解决方式：`Clone with HTTP`,  
命令`git clone https:/github.com/ Natsume0728/markdown.git`即可正常下载。
...
1. `git add [filename]or[dirname/]`后文件状态：  
__Changes to be committed,绿色字体显示new file__:  
![git_002.jpg](https://i.loli.net/2019/06/19/5d09c77fda45b30041.jpg)  
1. 修改文件001.html中的内容后执行`git status`命令，  
001.html文件状态为：  
__Changes not staged for commit__  
(文件已执行过`add`操作，理解为已纳入跟踪后做了修改，而未将修改后的文件提交到缓存区)，  
__红色字体显示modified__  
